...
void CMiniDumper::setPath(const wchar_t* path)
{
  memcpy_s(m_szSettingPath, MAX_PATH, path, MAX_PATH);
}
...
LONG CMiniDumper::writeMiniDump(_EXCEPTION_POINTERS *pExceptionInfo)
{
  ...
  if (GetModuleFileName(NULL, m_szAppPath, _MAX_PATH))
  {
    ...
    if (pSlash)
    {
      wcscpy_s(m_szAppBaseName, pSlash + 1);
    ...
    wcscat_s(szDbgHelpPath, _T("DBGHELP.DLL"));
